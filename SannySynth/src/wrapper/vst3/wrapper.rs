    ...
    unsafe fn get_parameter_info(
        ...
        *info = std::mem::zeroed();
        ...
        if P::MIDI_INPUT >= MidiConfig::MidiCCs && param_index >= num_actual_params {
            ...
            let is_program = self.inner.program_param_hash == Some(*param_hash);
            ...
            if is_program {
                info.flags |= ParameterFlags::kIsProgramChange as i32;
            }
        }
...
impl<P: Vst3Plugin> IUnitInfo for Wrapper<P> {
        ...
        match self.inner.param_units.info(unit_index as usize) {
            Some((unit_id, unit_info)) => {
                *info = mem::zeroed();
                ...
                info.program_list_id = if unit_id == kRootUnitId {
                    self.inner
                        .program_list
                        .as_ref()
                        .map(|_| 0)
    ...
    unsafe fn get_program_list_count(&self) -> i32 {
        self.inner.program_list.as_ref().map(|_| 1).unwrap_or(0)
    }
    ...
    unsafe fn get_program_list_info(
        ...
        let Some(program_list) = &self.inner.program_list else {
            return kInvalidArgument;
        };
        ...
        *info = mem::zeroed();
        let info = &mut *info;
        info.id = 0;
        info.program_count = program_list.list.programs.len() as i32;
        let name_ptr = &mut info.name as *mut _ as *mut [TChar; 128];
        u16strlcpy(unsafe { &mut *name_ptr }, &program_list.list.name);
    ...
    unsafe fn get_program_name(
        ...
        if program_index < 0 || program_index as usize >= program_list.list.programs.len() {
            return kInvalidArgument;
        }

        let program = &program_list.list.programs[program_index as usize];
        u16strlcpy(&mut *(name as *mut [TChar; 128]), &program.name);
