...
use crate::prelude::{
    ...
    Plugin, PluginNoteEvent, ProcessMode, ProcessStatus, ProgramList, TaskExecutor, Transport,
    Vst3Plugin,
};
...
pub(crate) struct WrapperInner<P: Vst3Plugin> {
    ...
    pub program_list: Option<ProgramListState>,
    pub program_param_hash: Option<u32>,
}

pub(crate) struct ProgramListState {
    pub list: ProgramList,
    pub presets: Vec<Vec<(u32, f32)>>,
}

impl ProgramListState {
    pub fn from_list(list: ProgramList, param_id_to_hash: &HashMap<String, u32>) -> Self {
            ...
            Some(param_ptr) => {
                ...
                if self.program_param_hash == Some(hash) {
                    if let Some(program_list) = &self.program_list {
                        let index = unsafe { param_ptr.preview_plain(normalized_value) } as usize;
                        let index = index.min(program_list.presets.len().saturating_sub(1));
