...
use crate::prelude::{
    ...
    ProcessMode, ProcessStatus, ProgramList, SysExMessage, TaskExecutor, Transport,
};
...
struct ProgramListState {
    #[allow(dead_code)]
    list: ProgramList,
    presets: Vec<Vec<(u32, f32)>>,
}

impl ProgramListState {
    fn from_list(list: ProgramList, param_id_to_hash: &HashMap<String, u32>) -> Self {
        let mut presets = Vec::with_capacity(list.programs.len());

        for program in &list.programs {
            let mut values = Vec::new();
            for (param_id, normalized) in &program.values {
                if let Some(hash) = param_id_to_hash.get(param_id) {
                    values.push((*hash, *normalized));
                }
            }
            presets.push(values);
        }
...
impl<P: ClapPlugin> Wrapper<P> {
    pub fn new(host_callback: *const clap_host) -> Arc<Self> {
        ...
        let program_list = plugin
            .program_list()
            .map(|list| ProgramListState::from_list(list, &param_id_to_hash));
        let program_param_hash = param_id_to_hash.get("program").copied();
            ...
            Some(param_ptr) => {
                match update_type {
                    ClapParamUpdate::PlainValueSet(clap_plain_value) => {
                        ...
                        if self.program_param_hash == Some(hash) {
                            if let Some(program_list) = &self.program_list {
                                let index = unsafe { param_ptr.preview_plain(normalized_value) };
                                let index = (index as usize)
                                    .min(program_list.presets.len().saturating_sub(1));
                                self.apply_program_by_index(index, sample_rate);
                            }
                        }
    ...
    pub fn apply_program_by_index(&self, index: usize, sample_rate: Option<f32>) {
        let Some(program_list) = &self.program_list else {
            return;
        };
        let Some(values) = program_list.presets.get(index) else {
            return;
        };

        for (hash, normalized_value) in values {
            if self.program_param_hash == Some(*hash) {
                continue;
            }
            ...
            (CLAP_CORE_EVENT_SPACE_ID, CLAP_EVENT_MIDI) => {
                ...
                match NoteEvent::from_midi(timing, &event.data) {
                    ...
                    Ok(note_event) if P::MIDI_INPUT >= MidiConfig::MidiCCs => {
                        ...
                        if let NoteEvent::MidiProgramChange { program, .. } = note_event {
                            if let Some(program_hash) = self.program_param_hash {
                                let sample_rate =
                                    self.current_buffer_config.load().map(|c| c.sample_rate);
                                self.update_plain_value_by_hash(
                                    program_hash,
                                    ClapParamUpdate::PlainValueSet(program as f64),
                                    sample_rate,
                                );
                                self.apply_program_by_index(program as usize, sample_rate);
                                handled = true;
                            }
                        }
                        if !handled {
