...
/// A single preset entry in a program list.
#[derive(Clone, Debug)]
pub struct ProgramPreset {
    pub name: String,
    /// Normalized parameter values keyed by the string parameter IDs.
    pub values: Vec<(String, f32)>,
}

/// A program list exposed to the host.
#[derive(Clone, Debug)]
pub struct ProgramList {
    pub name: String,
    pub programs: Vec<ProgramPreset>,
}
...
pub trait Plugin: Default + Send + 'static {
    ...
    /// Optional program list used by plugin APIs that support it.
