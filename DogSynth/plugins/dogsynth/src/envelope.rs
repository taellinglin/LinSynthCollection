...
impl ADSREnvelope {
    pub fn new(
        ...
        ADSREnvelope {
            ...
            decay,
            decay2,
            decay2_level: decay2_level.clamp(0.0, 1.0),
            sustain,
            release,
    ...
    pub fn set_decay2_level(&mut self, decay2_level: f32) {
        self.decay2_level = decay2_level.clamp(0.0, 1.0);
    }
    ...
    pub fn previous_value(&self) -> f32 {
        match self.state {
            ...
            ADSREnvelopeState::Decay => {
                1.0 - (1.0 - self.decay2_level) * (self.time / self.decay)
            }
            ADSREnvelopeState::Decay2 => {
                self.decay2_level
                    + (self.sustain - self.decay2_level) * (self.time / self.decay2)
            }
    ...
    pub fn advance(&mut self) {
        ...
        match self.state {
            ...
            ADSREnvelopeState::Decay2 if self.time >= self.decay2 => {
                self.state = ADSREnvelopeState::Sustain;
                self.time = 0.0;
            }
    ...
    pub fn get_decay2(&self) -> f32 {
        self.decay2
    }

    pub fn get_decay2_level(&self) -> f32 {
        self.decay2_level
    }
        ...
        let base_value = match self.state {
            ...
            ADSREnvelopeState::Decay2 => {
                if self.decay2 <= 0.0 {
                    ...
                    let linear = (self.time / self.decay2).min(1.0);
                    let curved = self.apply_tension(linear);
                    self.decay2_level + (self.sustain - self.decay2_level) * curved
                }
            }
            ADSREnvelopeState::Sustain => {
