...
impl LowpassFilter {
    ...
    fn update_coeffs(&mut self) {
        ...
        let resonance = self.resonance.clamp(0.0, 1.0);
        let q = 0.5 + resonance * 11.5;

        let w0 = 2.0 * PI * cutoff / self.sample_rate.max(1.0);
        let cos_w0 = w0.cos();
        let sin_w0 = w0.sin();
...
impl Filter for StatevariableFilter {
    fn process(&mut self, input: f32) -> f32 {
        ...
        let v1 = a1 * self.ic1eq + a2 * (input - self.ic2eq);
        let v2 = self.ic2eq + a2 * self.ic1eq + a3 * (input - self.ic2eq);
