...
struct SubSynthParams {
    ...
    #[persist = "morph_a"]
    morph_a_snapshot: Arc<RwLock<Option<String>>>,
    #[persist = "morph_b"]
    morph_b_snapshot: Arc<RwLock<Option<String>>>,
    #[id = "preset"]
    ...
    #[id = "add_mix"]
    additive_mix: FloatParam,
    #[id = "add_partials"]
    additive_partials: FloatParam,
    #[id = "add_tilt"]
    additive_tilt: FloatParam,
    #[id = "add_inharm"]
    additive_inharm: FloatParam,
    #[id = "add_morph"]
    additive_morph: FloatParam,
    #[id = "add_decay"]
    additive_decay: FloatParam,
    #[id = "add_drift"]
    additive_drift: FloatParam,
    #[id = "wt_custom"]
    ...
    #[id = "vel_add"]
    vel_additive_amount: FloatParam,
    #[id = "breath_en"]
    breath_enable: BoolParam,
    #[id = "breath_amt"]
    breath_amount: FloatParam,
    #[id = "breath_atk"]
    breath_attack_ms: FloatParam,
    #[id = "breath_dec"]
    breath_decay_ms: FloatParam,
    #[id = "breath_tone"]
    breath_tone: FloatParam,
    #[id = "sub_level"]
...
struct Voice {
    ...
    breath_filter: filter::BandpassFilter,
    pressure: f32,
...
impl Default for SubSynthParams {
    fn default() -> Self {
        Self {
            ...
            morph_a_snapshot: Arc::new(RwLock::new(None)),
            morph_b_snapshot: Arc::new(RwLock::new(None)),
            preset_index: IntParam::new("Preset", 0, IntRange::Linear { min: 0, max: 511 }),
            gain: FloatParam::new(
                "Gain",
            ...
            .with_step_size(0.01),
            additive_mix: FloatParam::new(
                "Additive Mix",
                0.0,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            .with_step_size(0.01),
            additive_partials: FloatParam::new(
                "Additive Partials",
                18.0,
                FloatRange::Linear { min: 1.0, max: 96.0 },
            )
            .with_step_size(1.0),
            additive_tilt: FloatParam::new(
                "Additive Tilt",
                0.0,
                FloatRange::Linear { min: -1.0, max: 1.0 },
            )
            .with_step_size(0.01),
            additive_inharm: FloatParam::new(
                "Additive Inharm",
                0.0,
                FloatRange::Linear { min: 0.0, max: 0.5 },
            )
            .with_step_size(0.01),
            additive_morph: FloatParam::new(
                "Additive Morph",
                0.25,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            .with_step_size(0.01),
            additive_decay: FloatParam::new(
                "Additive Decay",
                0.35,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            .with_step_size(0.01),
            additive_drift: FloatParam::new(
                "Additive Drift",
                0.12,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            ...
            .with_step_size(0.01),
            morph_amount: FloatParam::new(
                "Morph",
                0.0,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            .with_step_size(0.001),
            vel_additive_amount: FloatParam::new(
                "Vel Additive",
                0.0,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            ...
            .with_step_size(0.01),
            breath_enable: BoolParam::new("Breath Enable", false),
            breath_amount: FloatParam::new(
                "Breath Amount",
                0.25,
                FloatRange::Linear { min: 0.0, max: 1.0 },
            )
            .with_step_size(0.01),
            breath_attack_ms: FloatParam::new(
                "Breath Attack",
                2.0,
                FloatRange::Skewed {
                    min: 0.0,
            ...
            .with_step_size(0.1)
            .with_unit(" ms"),
            breath_decay_ms: FloatParam::new(
                "Breath Decay",
                180.0,
                FloatRange::Skewed {
                    min: 10.0,
                        ...
                        match event {
                            NoteEvent::NoteOn {
                                ...
                                let fm_envelope = ADSREnvelope::new(
                                    Self::ms_to_s(self.params.fm_env_attack_ms.value()),
                                    self.params.fm_env_amount.value(),
                                    Self::ms_to_s(self.params.fm_env_decay_ms.value()),
                                    self.params.fm_env_sustain_level.value(),
                                    Self::ms_to_s(self.params.fm_env_release_ms.value()),
                                    sample_rate,
                                    velocity,
                                    0.0,
                                );
                                let dist_envelope = ADSREnvelope::new(
                                    Self::ms_to_s(self.params.dist_env_attack_ms.value()),
                                    self.params.dist_env_amount.value(),
                                    Self::ms_to_s(self.params.dist_env_decay_ms.value()),
                                    self.params.dist_env_sustain_level.value(),
                                    Self::ms_to_s(self.params.dist_env_release_ms.value()),
                                    sample_rate,
                                    velocity,
                                    0.0,
                                );
                                let breath_envelope =
                                    self.construct_breath_envelope(sample_rate, velocity);
                                {
                            ...
                            NoteEvent::MidiProgramChange { program, .. } => {
                                let normalized = self
                                    .params
                                    .preset_index
                                    .preview_normalized(program as i32);
                                unsafe {
                                    self.params
                                        .preset_index
                                        .as_ptr()
                                ...
                                self.apply_preset(program as usize, sample_rate);
                            }
            ...
            for (value_idx, sample_idx) in (block_start..block_end).enumerate() {
                ...
                for voice in self.voices.iter_mut() {
                    if let Some(voice) = voice {
                        ...
                        let additive_mix_base = self.params.additive_mix.value();
                        let additive_partials = self.params.additive_partials.value();
                        let additive_tilt_base = self.params.additive_tilt.value();
                        let additive_inharm = self.params.additive_inharm.value();
                        let vel_additive_amount = self.params.vel_additive_amount.value();
                        ...
                        let breath_enable = self.params.breath_enable.value();
                        let breath_amount = self.params.breath_amount.value();
                        let breath_tone = self.params.breath_tone.value();
                        ...
                        let breath_env_value = voice.breath_envelope.get_value();
                        ...
                        let mut additive_mix_mod = 0.0;
                        let mut additive_tilt_mod = 0.0;
                        let mut additive_morph_mod = 0.0;
                        let mut additive_decay_mod = 0.0;
                        let mut additive_drift_mod = 0.0;
                        ...
                        | {
                            ...
                            match target {
                                ...
                                ModTarget::AdditiveMix => additive_mix_mod += mod_value,
                                ModTarget::AdditiveTilt => additive_tilt_mod += mod_value,
                                ModTarget::AdditiveMorph => additive_morph_mod += mod_value,
                                ModTarget::AdditiveDecay => additive_decay_mod += mod_value,
                                ModTarget::AdditiveDrift => additive_drift_mod += mod_value,
                        ...
                        let vel = voice.velocity.clamp(0.0, 1.0);
                        let vel_curve = vel * vel;
                        let vel_add = vel_additive_amount * vel_curve;
                        let additive_mix =
                            (additive_mix_base + additive_mix_mod + vel_add * 0.25)
                                .clamp(0.0, 1.0);
                        let additive_tilt =
                            (additive_tilt_base + additive_tilt_mod + vel_add * 0.3)
                                .clamp(-1.0, 1.0);
                        let additive_morph =
                            (self.params.additive_morph.value() + additive_morph_mod)
                                .clamp(0.0, 1.0);
                        let additive_decay =
                            (self.params.additive_decay.value() + additive_decay_mod)
                                .clamp(0.0, 1.0);
                        let additive_drift =
                            (self.params.additive_drift.value() + additive_drift_mod)
                                .clamp(0.0, 1.0);
                        ...
                        let additive_partials = additive_partials.round().clamp(1.0, 96.0) as usize;
                        ...
                        for i in 0..unison_count {
                            ...
                            let additive_sample = generate_additive_sample_advanced(
                                classic_phase,
                                additive_partials,
                                additive_tilt,
                                additive_inharm,
                        ...
                        let additive_sum = additive_sum / unison_count as f32;
                        ...
                        if additive_mix > 0.0 {
                            generated_sample = generated_sample * (1.0 - additive_mix)
                                + additive_sum * additive_mix;
                        }
                        if sub_level > 0.0 {
                        ...
                        if breath_enable && breath_amount > 0.0 {
                            let mut breath_sample = self.prng.gen::<f32>() * 2.0 - 1.0;
                            voice.breath_filter.set_cutoff(breath_tone);
                            voice.breath_filter.set_resonance(0.6);
                            breath_sample = voice.breath_filter.process(breath_sample);
                            generated_sample += breath_sample
                                * breath_env_value
                                * breath_amount
                                * 0.6;
                        }
                        ...
                        let filtered_sample = if matches!(voice.filter.unwrap(), FilterType::None) {
                            filtered_sample
                        } else {
...
impl SubSynth {
    ...
    fn refresh_morph_snapshots(&mut self) -> bool {
        ...
        if let Ok(snapshot) = self.params.morph_a_snapshot.read() {
            if self.morph_a_json != *snapshot {
                self.morph_a_json = snapshot.clone();
                self.morph_a_snapshot = snapshot
                    .as_deref()
        ...
        if let Ok(snapshot) = self.params.morph_b_snapshot.read() {
            if self.morph_b_json != *snapshot {
                self.morph_b_json = snapshot.clone();
                self.morph_b_snapshot = snapshot
                    .as_deref()
                    .and_then(|json| serde_json::from_str(json).ok());
                changed = true;
            }
    ...
    fn apply_preset(&mut self, index: usize, sample_rate: f32) {
        ...
        preset.data.apply_direct(&self.params);
        unsafe {
            self.params
                .gain
                .as_ptr()
                .update_smoother(sample_rate, false);
        }
    ...
    fn construct_envelopes(
        ...
        let amp_attack = Self::ms_to_s(self.params.amp_attack_ms.value());
        let amp_decay = Self::ms_to_s(self.params.amp_decay_ms.value());
        let amp_release = Self::ms_to_s(self.params.amp_release_ms.value());
        let cut_attack = Self::ms_to_s(self.params.filter_cut_attack_ms.value());
        let cut_decay = Self::ms_to_s(self.params.filter_cut_decay_ms.value());
        let cut_release = Self::ms_to_s(self.params.filter_cut_release_ms.value());
        let res_attack = Self::ms_to_s(self.params.filter_res_attack_ms.value());
        let res_decay = Self::ms_to_s(self.params.filter_res_decay_ms.value());
        let res_release = Self::ms_to_s(self.params.filter_res_release_ms.value());
        (
            ADSREnvelope::new(
                amp_attack,
                0.0,
    ...
    fn construct_breath_envelope(&self, sample_rate: f32, velocity: f32) -> ADSREnvelope {
        let attack = Self::ms_to_s(self.params.breath_attack_ms.value());
        let decay = Self::ms_to_s(self.params.breath_decay_ms.value());
        ADSREnvelope::new(
            attack,
            0.0,
            decay,
    ...
    fn start_voice(
        ...
        let new_voice = Voice {
            ...
            breath_filter: filter::BandpassFilter::new(2400.0, 0.6, sample_rate),
            vib_mod,
            trem_mod,
            mod_lfo1,
            mod_lfo2,
        ...
        if let Some(free_voice_idx) = self.voices.iter().position(|voice| voice.is_none()) {
            ...
            if voice.is_none() {
                *voice = Some(new_voice);
                ...
                voice.breath_envelope.set_envelope_stage(ADSREnvelopeState::Attack);
                voice.vib_mod.trigger();
                voice.trem_mod.trigger();
                voice.mod_lfo1.trigger();
                voice.mod_lfo2.trigger();
                ...
                oldest_voice.amp_envelope.get_state() == ADSREnvelopeState::Release
            {
                ...
                *oldest_voice = new_voice;
                ...
                oldest_voice.breath_envelope.set_envelope_stage(ADSREnvelopeState::Attack);
        ...
        for voice in &mut self.voices {
            if let Some(voice) = voice {
                if voice_id == Some(voice.voice_id) || (channel == voice.channel && note == voice.note) {
                    ...
                    voice.breath_envelope.set_envelope_stage(ADSREnvelopeState::Release);
                }
            }
        }
    }
    ...
    fn find_or_create_voice(
        ...
        let mut new_voice = Voice {
            ...
            breath_envelope: ADSREnvelope::new(
                Self::ms_to_s(self.params.breath_attack_ms.value()),
                0.0,
                Self::ms_to_s(self.params.breath_decay_ms.value()),
                0.0,
                Self::ms_to_s(self.params.breath_decay_ms.value()) * 0.5,
                44100.0,
                1.0,
                0.0,
            ),
            ...
            breath_filter: filter::BandpassFilter::new(2400.0, 0.6, 192000.0),
            pan,
            pressure,
            brightness,
            expression,
        ...
        new_voice.breath_envelope.trigger();
