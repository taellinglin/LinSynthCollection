...
use crate::util::{permit_alloc, NOTES};
...
impl<P: Vst3Plugin> IUnitInfo for Wrapper<P> {
        ...
        match self.inner.param_units.info(unit_index as usize) {
            Some((unit_id, unit_info)) => {
                *info = mem::zeroed();
                ...
                info.program_list_id = if P::MIDI_INPUT >= MidiConfig::Basic {
                    0
                } else {
                    kNoProgramListId
                };
    ...
    unsafe fn get_program_list_info(
        ...
        check_null_ptr!(_info);
        if _list_index != 0 || P::MIDI_INPUT < MidiConfig::Basic {
            return kInvalidArgument;
        }
        ...
        *info = mem::zeroed();
        ...
        let name_slice = std::slice::from_raw_parts_mut(
            info.name.as_mut_ptr() as *mut TChar,
            info.name.len(),
        );
        u16strlcpy(name_slice, "Drum Map");
        kResultOk
    }
    ...
    unsafe fn get_program_name(
        ...
        check_null_ptr!(_name);
        if _list_id != 0 || _program_index != 0 || P::MIDI_INPUT < MidiConfig::Basic {
            return kInvalidArgument;
        }
        let name_slice = std::slice::from_raw_parts_mut(_name as *mut TChar, 128);
        u16strlcpy(name_slice, "Default");
        kResultOk
    }
    ...
    unsafe fn get_program_pitch_name(
        ...
        let name = if is_drum_plugin_name(P::NAME) {
            gm_drum_name(_pitch)
                .map(|label| label.to_string())
                .unwrap_or_else(|| note_name(_pitch as u8))
        } else {
            note_name(_pitch as u8)
        };

        let name_slice = std::slice::from_raw_parts_mut(_name as *mut TChar, 128);
        u16strlcpy(name_slice, &name);
        kResultOk
    }
...
fn is_drum_plugin_name(name: &str) -> bool {
    let lowered = name.to_ascii_lowercase();
    lowered.contains("drum")
}

fn gm_drum_name(pitch: i16) -> Option<&'static str> {
    match pitch as u8 {
