# PlantSynth - Complete Documentation

[![Automated builds](https://github.com/taellinglin/PlantSynth/actions/workflows/build.yml/badge.svg?branch=master)](https://github.com/taellinglin/PlantSynth/actions/workflows/build.yml?query=branch%3Amaster)
[![Tests](https://github.com/taellinglin/PlantSynth/actions/workflows/test.yml/badge.svg?branch=master)](https://github.com/taellinglin/PlantSynth/actions/workflows/test.yml?query=branch%3Amaster)
[![Documentation](https://github.com/taellinglin/PlantSynth/actions/workflows/docs.yml/badge.svg?branch=master)](https://github.com/taellinglin/PlantSynth/actions/workflows/docs.yml?query=branch%3Amaster)

---

## Overview

**PlantSynth** is a knitty gritty wavetable bass synthesizer implemented as a VST3/CLAP plugin, built with the NIH-plug framework. It blends aggressive modulation, sub layering, and wavetable distortion to deliver dubstep-ready bass and heavy electronic sound design.

### Key Features

- 🎹 **16-Voice Polyphony** - Smooth, responsive voice management
- 🌊 **Multiple Waveforms** - Sine, Square, Sawtooth, Triangle, Pulse, and Noise
- 🎚️ **Full ADSR Envelopes** - Independent control for amplitude and filter parameters
- 🔥 **Wavetable Distortion** - Wavefolding drive for gritty harmonics
- 🔊 **Bass-Focused Filtering** - Low-pass, High-pass, Band-pass, Notch, Statevariable
- 🧱 **Sub Layer** - Dedicated sub level for weight and punch
...
#### Windows
1. Download the latest release from the [Releases page](https://github.com/taellinglin/PlantSynth/releases)
2. Extract the archive
3. Copy `PlantSynth.vst3` folder to: `C:\Program Files\Common Files\VST3\`
4. Copy `PlantSynth.clap` file to: `C:\Program Files\Common Files\CLAP\`
...
4. Copy `PlantSynth.vst3` to: `~/Library/Audio/Plug-Ins/VST3`
5. Copy `PlantSynth.clap` to: `~/Library/Audio/Plug-Ins/CLAP`
6. Restart your DAW
...
2. Extract and copy files:
   ```bash
   mkdir -p ~/.vst3 ~/.clap
  cp -r PlantSynth.vst3 ~/.vst3/
  cp PlantSynth.clap ~/.clap/
  chmod +x ~/.vst3/PlantSynth.vst3/Contents/x86_64-linux/PlantSynth.so
  chmod +x ~/.clap/PlantSynth.clap
   ```
...
git clone https://github.com/taellinglin/PlantSynth.git
cd PlantSynth

# Build release version
cargo xtask bundle PlantSynth --release
```

The compiled plugins will be located in `target/bundled/`:
- `PlantSynth.vst3` - VST3 plugin
- `PlantSynth.clap` - CLAP plugin

### Development Build

```bash
# Build debug version (faster compilation, slower runtime)
cargo xtask bundle PlantSynth
...
PlantSynth features a clean, organized interface using the custom **zCool XiaoWei** font for enhanced readability.
...
- Verify bundle is not quarantined: `xattr -d com.apple.quarantine PlantSynth.vst3`

**Linux:**
- Check file permissions (chmod +x)
- Verify shared library dependencies: `ldd PlantSynth.so`
...
```
PlantSynth/
├── plugins/PlantSynth/     # Main plugin code
│   └── src/
...
- **GitHub Repository**: https://github.com/taellinglin/PlantSynth
- **Issues/Bug Reports**: https://github.com/taellinglin/PlantSynth/issues
...
*Documentation generated for PlantSynth v0.1.0*
