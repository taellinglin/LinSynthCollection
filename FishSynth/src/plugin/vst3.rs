...
pub trait Vst3Plugin: Plugin {
    ...
    /// Optional VST3 program list name. Hosts can use this to label program selectors.
    const VST3_PROGRAM_LIST_NAME: &'static str = "Programs";

    /// Optional VST3 program list. When empty, the wrapper reports no program lists.
    const VST3_PROGRAM_NAMES: &'static [&'static str] = &[];
}
