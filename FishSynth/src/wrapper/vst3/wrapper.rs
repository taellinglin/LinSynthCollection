...
use super::util::{
    u16strlcpy, u16strlcpy_u16, VstPtr, VST3_MIDI_CCS, VST3_MIDI_NUM_PARAMS,
...
impl<P: Vst3Plugin> IUnitInfo for Wrapper<P> {
        ...
        match self.inner.param_units.info(unit_index as usize) {
            Some((unit_id, unit_info)) => {
                *info = mem::zeroed();
                ...
                info.program_list_id = if P::VST3_PROGRAM_NAMES.is_empty() {
                    kNoProgramListId
                } else {
                    0
                };
    ...
    unsafe fn get_program_list_count(&self) -> i32 {
        if P::VST3_PROGRAM_NAMES.is_empty() {
            0
        } else {
            1
        }
    ...
    unsafe fn get_program_list_info(
        ...
        *info = mem::zeroed();
        let info = &mut *info;
        info.id = 0;
        info.program_count = P::VST3_PROGRAM_NAMES.len() as i32;
        u16strlcpy_u16(&mut info.name, P::VST3_PROGRAM_LIST_NAME);
    ...
    unsafe fn get_program_name(
        ...
        if P::VST3_PROGRAM_NAMES.is_empty() || list_id != 0 {
            return kInvalidArgument;
        }

        let program_index = program_index as isize;
        if program_index < 0
            || (program_index as usize) >= P::VST3_PROGRAM_NAMES.len()
            || name.is_null()
        {
            return kInvalidArgument;
        }
        ...
        u16strlcpy_u16(
            std::slice::from_raw_parts_mut(name, 128),
            P::VST3_PROGRAM_NAMES[program_index as usize],
        );
