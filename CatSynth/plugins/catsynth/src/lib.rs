...
impl Plugin for DrumSynth {
    ...
    fn process(
        ...
        let seq_enabled = self.params.sequencer.enabled.value();
        let seq_rate = self.params.sequencer.rate.value();
        let seq_swing = self.params.sequencer.swing.value();
        let tempo = context.transport().tempo.unwrap_or(120.0) as f32;
        ...
        if seq_enabled {
            ...
            for sample_idx in 0..num_samples {
                if let Some(step) =
                    self.sequencer
                        .advance(sample_rate, tempo, seq_rate, seq_swing)
                {
                    if sample_idx > segment_start {
                        let range = segment_start..sample_idx;
                        let mut aux_outputs = collect_aux_outputs(aux, range.clone());
                        let aux_slice = if aux_outputs.is_empty() {
                            None
                        } else {
                            Some(aux_outputs.as_mut_slice())
                        };
                        self.engine
                            .process(&mut left[range.clone()], &mut right[range], aux_slice);
                    }
            ...
            if segment_start < num_samples {
                let range = segment_start..num_samples;
                let mut aux_outputs = collect_aux_outputs(aux, range.clone());
                let aux_slice = if aux_outputs.is_empty() {
                    None
                } else {
            ...
            self.engine.process(left, right, aux_slice);
        }
